package work.gaigeshen.formwork.basal.bpmn.service.flowable;
...
import work.gaigeshen.formwork.basal.bpmn.candidate.*;
import work.gaigeshen.formwork.basal.bpmn.process.Process;
import work.gaigeshen.formwork.basal.bpmn.process.*;
import work.gaigeshen.formwork.basal.bpmn.service.BpmnService;
import work.gaigeshen.formwork.basal.bpmn.usertask.*;
...
public class FlowableBpmnService implements BpmnService {
    ...
    public Collection<Process> queryProcesses(ProcessQueryParameters parameters) {
        ...
        if (Objects.nonNull(parameters.getProcessId())) {
            processInstanceQuery.processDefinitionKey(FlowableBpmnParser.wrapProcessId(parameters.getProcessId()));
        }
        ...
        for (HistoricProcessInstance processInstance : processInstances) {
            DefaultProcess process = DefaultProcess.builder()
                    .processId(FlowableBpmnParser.unWrapProcessId(processInstance.getProcessDefinitionKey()))
    ...
    public Collection<UserTask> queryTasks(UserTaskQueryParameters parameters) {
        ...
        if (Objects.nonNull(parameters.getProcessId())) {
            taskQuery.processDefinitionKey(FlowableBpmnParser.wrapProcessId(parameters.getProcessId()));
        }
        if (Objects.nonNull(parameters.getBusinessKey())) {
        ...
        for (Task task : queryResult) {
            ...
            if (Objects.isNull(processId) || Objects.isNull(businessKey)) {
                ProcessInstance process = runtimeService.createProcessInstanceQuery()
                        ...
                        .singleResult();
                processId = FlowableBpmnParser.unWrapProcessId(process.getProcessDefinitionKey());
    ...
    public Collection<UserHistoricTask> queryHistoricTasks(UserHistoricTaskQueryParameters parameters) {
        ...
        if (Objects.nonNull(parameters.getProcessId())) {
            historicTaskInstanceQuery.processDefinitionKey(FlowableBpmnParser.wrapProcessId(parameters.getProcessId()));
        }
        if (Objects.nonNull(parameters.getBusinessKey())) {
        ...
        for (HistoricTaskInstance historicTask : queryResult) {
            ...
            if (Objects.isNull(processId) || Objects.isNull(businessKey)) {
                HistoricProcessInstance historicProcess = historyService.createHistoricProcessInstanceQuery()
                        ...
                        .singleResult();
                processId = FlowableBpmnParser.unWrapProcessId(historicProcess.getProcessDefinitionKey());
    ...
    public List<UserTaskActivity> queryTaskActivities(UserTaskActivityQueryParameters parameters) {
        ...
        HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
                .processDefinitionKey(FlowableBpmnParser.wrapProcessId(parameters.getProcessId()))
        ...
        taskActivities.add(
                UserTaskActivity.builder()
                        ...
                        .startTime(startTime).endTime(startTime).status(UserTaskActivity.Status.APPROVED)
                        .build()
        );
        ...
        for (HistoricActivityInstance activity : activities) {
            ...
            if (Objects.isNull(activity.getEndTime())) {
                ...
                UserTaskActivity lastActivityCandidate = UserTaskActivity.builder()
                        ...
                        .startTime(activity.getStartTime()).status(UserTaskActivity.Status.PROCESSING)
                        .build();
                ...
                UserTaskActivity userTaskActivity = UserTaskActivity.builder()
                        .status(((boolean) rejected) ? UserTaskActivity.Status.REJECTED : UserTaskActivity.Status.APPROVED)
    ...
    public UserTaskAutoCompletion autoCompleteTasks(UserTaskAutoCompleteParameters parameters) {
        ...
        String wrappedProcessId = FlowableBpmnParser.wrapProcessId(parameters.getProcessId());
        String businessKey = parameters.getBusinessKey();
    ...
    public UserTaskAutoCompletion startProcess(ProcessStartParameters parameters) {
        ...
        String processId = FlowableBpmnParser.wrapProcessId(parameters.getProcessId());
        String businessKey = parameters.getBusinessKey();
        Map<String, Object> variables = parameters.getVariables();
    ...
    private Map<String, Map<String, Object>> getProcessTaskVariables(String processId, String businessKey) {
        HistoricProcessInstance processInstance = historyService.createHistoricProcessInstanceQuery()
                .processDefinitionKey(FlowableBpmnParser.wrapProcessId(processId))
                .processInstanceBusinessKey(businessKey)
                .singleResult();
    ...
    private TypedCandidate getTaskCandidate(String taskId) {
        ...
        return FlowableBpmnParser.getCandidate(getTaskFlowNode(processDefinitionId, taskDefinitionKey));
    }
    ...
    private List<TypedCandidate> getProcessCalculateCandidates(String processId, Map<String, Object> variables) {
        ...
        return FlowableBpmnParser.getCandidates(getProcessStarterFlowNode(processDefinitionId), variables);
    }
    ...
    private void applyProcessTaskCandidates(String processId, String businessKey) {
        ...
        getProcessTaskVariables(processId, businessKey).forEach((taskId, vars) -> {
            ...
            if (vars.containsKey("appliedCandidate")) {
                if (CandidateType.STARTER_APPOINTEE == definedCandidate.getType()) {
